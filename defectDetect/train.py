...
def hill_climb_ensemble(val_pred_df: pd.DataFrame, y_val: pd.Series):
    STOP = False
    current_best_ensemble = val_pred_df.iloc[:, 0]
    MODELS = val_pred_df.iloc[:, 1:]
    weight_range = np.arange(0.1, 0.5, 0.01)
    model_weights = {}
    history = {val_pred_df.columns[0]: roc_auc_score(y_val, current_best_ensemble)}
    i = 0
    ...
    while not STOP:
        ...
        if k_best is not None:
            ...
            history["".join(list(history.keys())[-1]) + "+" + k_best] = potential_new_best_cv_score
        else:
            STOP = True
...
def voting_ensemble(models, X_train, y_train, X_val):
...
def train(X_train, y_train, X_val, y_val):
    ...
    model_weights, history, val_hill_climb_preds = hill_climb_ensemble(val_pred_df, y_val)
    ...
    votclf_model, val_votclf_preds = voting_ensemble(models, X_train, y_train, X_val)

    final_val_preds = (val_hill_climb_preds + val_votclf_preds) / 2

    score = roc_auc_score(y_val, final_val_preds)
    print(score)
...
if __name__ == "__main__":
    ...
    X_train, X_val, y_train, y_val = data.data_split(df)
